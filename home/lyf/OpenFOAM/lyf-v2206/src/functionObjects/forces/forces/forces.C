...
Foam::functionObjects::forces::devRhoReff() const
{
    ...
    typedef incompressible::phaseIncompressibleTurbulenceModel VofTurbModel;

    if (foundObject<cmpTurbModel>(cmpTurbModel::propertiesName))
    {
        Info << "printing 执行devRhoReff的过程1" << endl;
        const auto& turb =
            lookupObject<cmpTurbModel>(cmpTurbModel::propertiesName);

        return turb.devRhoReff();
    }
    else if (foundObject<VofTurbModel>(VofTurbModel::propertiesName))
    {
        Info << "printing 执行devRhoReff的过程2" << endl;
        const auto& turb =
            lookupObject<VofTurbModel>(VofTurbModel::propertiesName);

        return turb.devRhoReff();
    }
    else if (foundObject<icoTurbModel>(icoTurbModel::propertiesName))
    {
        Info << "printing 执行devRhoReff的过程3" << endl;
        const auto& turb =
    ...
    {
        Info << "printing 执行devRhoReff的过程4" << endl;
    ...
    {
        Info << "printing 执行devRhoReff的过程5" << endl;
        const auto& laminarT =
    ...
    {
        Info << "printing 执行devRhoReff的过程6" << endl;
