...
// #   include "createExternalWaveForcing.H"
    ...
    const dimensionedScalar& rho2 = dimensionedScalar(sDA.lookup("rho"));

    const dictionary& sDW(transProp.subDict(Foam::waves2Foam::waterPhase()));
    const dimensionedScalar& rho1 = dimensionedScalar(sDW.lookup("rho"));
    ...
    if (waveProperties.get<scalar>("totalPressure") > 0)
    {
    ...
    else
    {
        FatalErrorIn
        (
            "setCompressibleWaveField.C"
        )   << "对于可压缩的情况，constant/waveProperties.input中totalPressure应设置为大气压"
            << exit(FatalError);
    }
