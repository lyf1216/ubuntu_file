...
:
    ...
    name_
    (
       (U.db().lookupObject<IOdictionary>("waveProperties")).get<word>("initializationName")
    ),
...
void setWaveField::correct()
{
    ...
    forAll (U_, celli)
    {
        ...
        scalar pTarget
        (
            (U_.db().lookupObject<IOdictionary>("waveProperties"))
            .getOrDefault<scalar>("totalPressure",0.0)
        );
    ...
    forAll (mesh_.boundaryMesh(), patchI)
    {
        ...
        std::string pStr(pd_.boundaryField()[patchI].type().c_str());
        ...
        if (pType == "gabc")
        {
            ...
            pd_.boundaryFieldRef()[patchI].evaluate();
//            Info << pd_.boundaryField()[patchI].type() << endl;
        }
    }
